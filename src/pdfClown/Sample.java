...
public abstract class Sample {
	...
	protected void prompt(String message) {
		...
		/* Utils.prompt(message); */} // gör om lite här
	...
	protected int promptPageChoice(String inputDescription, int startIndex, int endIndex) {
		...
		try {
			pageIndex = Integer
					.parseInt(promptChoice(inputDescription + " [" + (startIndex + 1) + "-" + endIndex + "]: ")) - 1;
		} catch (Exception e) {
	 ...
	 * @return Serialization path.
	 */
	protected String serialize(File file, SerializationModeEnum serializationMode, String title, String subject,
			String keywords) {
	 ...
	 * @return Serialization path.
	 */
	protected String serialize(File file, String fileName, SerializationModeEnum serializationMode, String title,
		...
		java.io.File outputFile = new java.io.File(
				outputPath + java.io.File.separator + fileName + "." + serializationMode + ".pdf");
