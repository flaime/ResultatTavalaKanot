...
public class ParseToObjekt {
	...
	public Tävling ParseToTävling (String data) throws WrongFormatPDFParseException{
		...
		if(datumSplit[0].trim().toLowerCase().contains("Resultatlista".toLowerCase())){
			...
			typ = "Startlista";
			throw new WrongFormatPDFParseException("Programet stödjer än inte startlistor i PDF form (men i databasform) :( Men säg gärna till om du vill hjälpa till :)");
		}else
...
//				System.out.println("---");
	...
	private String rensaData(String data){
		...
		//ja du kan göras bättre alltid men för räcka så nu
	...
	private ArrayList<String> delaUppILopp(String data){
		...
		for (int i = rader.length-1; i >= 0 ; i--) {
			if(rader[i].trim().matches("\\d\\d\\d\\d-\\d\\d-\\d\\d")){
				lopp += rader[i]+"\n";
				loppen.add(vändPåRadernasÅrdning(lopp)); //dä loppen är bakofram och de hamnar "sista" först..
				lopp = "";
			}else
				lopp += rader[i]+"\n";
