...
public class LoppVisarenKontroller {
	...
	private void handleDeleteLopp() {
		...
		if (selectedIndex >= 0) {
			...
			alert.setContentText("Vänligan markera ett lopp i tabellen.");// Please
																			// select
																			// a
																			// person
																			// in
	...
	private void initialize() {
		...
		// changed.
		loppTable.getSelectionModel().selectedItemProperty()
				.addListener((observable, oldValue, newValue) -> visaLoppDetails(newValue));
	}
	 ...
	 */
	/*
	 * @FXML private TableColumn<Lopp, String> BanaNummer;
	 * 
	 * @FXML private TableColumn<Lopp, String> BanaNamn;
	 * 
	 * @FXML private TableColumn<Lopp, String> BanaKlubb;
	 * 
	 * @FXML private TableColumn<Lopp, String> BanaTid;
	 */
	private void visaLoppDetails(Lopp lopp) {
		if (lopp != null) {
			...
			// todo impelmentra det exemplet på bättre sätt att andvända
			// "riktig" dataklass och sen köra konventering istället ....
			...
			/**
			 * nu blir det lite "repetetivt" men för att man ska kunna editera
			 * alla kolumner/detajler på en bana...
			 */
						...
						public void handle(CellEditEvent<Bana, String> t) {
							if (lopp.FinnsBanaMedNummer(t.getNewValue()) == false) {
								...
								if (villDuFortsättaÄvenOm("Bana", t.getNewValue())) {
									...
									// gör det jag går nedan för att den ska
									// uppdatera och märka att värdet inte
									// ändrades...
						...
						public void handle(CellEditEvent<Bana, String> t) {
							if (lopp.FinnsBanaMedNamnet(t.getNewValue()) == false) {
								...
								if (villDuFortsättaÄvenOm("namn", t.getNewValue())) {
									...
									// behåller det gamla värdet
									// går det jag går nedan för att den ska
									// uppdatera och märka att värdet inte
									// Ändrades...
									t.getTableColumn().setVisible(false);
									t.getTableColumn().setVisible(true);
						...
						public void handle(CellEditEvent<Bana, String> t) {
							if (lopp.FinnsBanaMedKLubben(t.getNewValue()) == false) {
								...
								if (villDuFortsättaÄvenOm("klubb", t.getNewValue())) {
									...
									// behåller det gamla värdet
									// gör det jag gör nedan för att den ska
									// uppdatera och märka att värdet inte
									// Ändrades...
									t.getTableColumn().setVisible(false);
									t.getTableColumn().setVisible(true);
			...
			BanaTid.setOnEditCommit(// setOnEditCommit(
					new EventHandler<CellEditEvent<Bana, String>>() {
						...
						public void handle(CellEditEvent<Bana, String> t) {
							...
							alert.setTitle("Söker");
							alert.setHeaderText("Är du söker på att det är korekt format på tiden?");
							...
							if (result.get() == ButtonType.OK) {
								...
								// behåller det gamla värdet
								// gåxr det jag går nedan för att den ska
								// uppdatera och märka att värdet inte
								// ändrades...
								t.getTableColumn().setVisible(false);
								t.getTableColumn().setVisible(true);
	...
	protected boolean villDuFortsättaÄvenOm(String omVad, String skaViÄndraTillDetta) {
		...
		alert.setHeaderText(
				"Vill du fortsätta och spara " + omVad + " som nu är " + skaViÄndraTillDetta + " men redan finns?");
	...
	private void handleRemoveBana() {
		...
		if (selectedIndex >= 0) {
			...
			alert.setHeaderText("Ingen bana �r markerad");
			alert.setContentText("V�nligen markera en banan.");
